...
class MainViewModel(application: Application): AndroidViewModel(application) {
    ...
    val countBooleanLiveData: LiveData<String> = SharedPreferenceBooleanLiveData(preferences, "count_boolean", true)
        .map { it.toString() }

    val countLongLiveData: LiveData<String> = SharedPreferenceLongLiveData(preferences, "count_long", 0L)
        .map { it.toString() }

    val countIntLiveData: LiveData<String> = SharedPreferenceIntLiveData(preferences, "count_int", 0)
        .map { it.toString() }

    val countFloatLiveData: LiveData<String> = SharedPreferenceFloatLiveData(preferences, "count_float", 0.0f)
        .map { it.toString() }

    val countStringLiveData: LiveData<String> = SharedPreferenceStringLiveData(preferences, "count_string", "0")
        .map { it ?: "null" }

    val countStringSetLiveData: LiveData<String> = SharedPreferenceStringSetLiveData(preferences, "count_string_set", setOf("a", "b", "c"))
    ...
    init {
        viewModelScope.launch {
            while (true) {
                ...
                val countStringSet = preferences.getStringSet("count_string_set", setOf("a", "b", "c"))!!
